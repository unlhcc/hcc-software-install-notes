...
rm -rf build_mpi/
mkdir build_mpi && pushd build_mpi
cmake -DCMAKE_INSTALL_PREFIX=${PREFIX}  -DCMAKE_C_COMPILER=mpicc -DCMAKE_CXX_COMPILER=mpic++ \
      -DGMX_MPI=ON -DGMX_DOUBLE=ON -DGMX_BUILD_OWN_FFTW=OFF -DGMX_DEFAULT_SUFFIX=OFF -DGMX_BINARY_SUFFIX="" -DGMX_LIBS_SUFFIX="" \
      -DCMAKE_PREFIX_PATH=/util/opt/hwloc/2.1/gcc/4.8 -DGMX_BUILD_MDRUN_ONLY=on ../
#make
#make install
